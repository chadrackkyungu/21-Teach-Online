...
const ProfileMenu = props => {
  ...
  const userDet = useStore1Selector(user); console.log(userDet.user?.pic);
    ...
    <React.Fragment>
      <Dropdown isOpen={menu} toggle={() => setMenu(!menu)} className="d-inline-block">
        ...
        <DropdownToggle className="btn header-item waves-effect" id="page-header-user-dropdown" tag="button">
          <img className="rounded-circle header-profile-user" src={userDet.user.pic ? userDet.user?.pic : userDet.user.pic} alt="Avatar"/>
          <span className="text-white"> {userDet.user?.name} </span>
        </DropdownToggle>
